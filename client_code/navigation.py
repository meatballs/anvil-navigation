...
def build_menu(container, items, with_title=True):
  ...
  if with_title:
    get_open_form().add_component(_title_label, slot="title")
  for item in items:
...
def navigation_link(text, routing="classic", target=None, icon=None, on_click=None, visibility=None):
